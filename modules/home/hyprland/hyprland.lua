...
package.path = home .. "/.cache/wallust/?.lua;" .. home .. "/.config/hypr/?.lua;" .. package.path

----------------------
--- WALLUST COLORS ---
----------------------

local colors = require("colors-hypr")
...
hl.on("hyprland.start", function()
    hl.exec_cmd("waybar & swaync & swayosd-server & swww")
...
hl.config({
    general = {
        ...
        col = {
            active_border = { colors = { colors.color4, colors.color3, colors.color2 }, angle = 45 },
...
hl.animation({ leaf = "workspaces", enabled = true, speed = 4.5, bezier = "easeOutQuint", style = "slidefade 15%" })
...
-- hl.gesture({ fingers = 3, direction = "up", action = "expo", mode = "on" })
-- hl.gesture({ fingers = 3, direction = "down", action = "expo", mode = "off" })
...
-- Stop scroll binds from focusing on windows between scrolls
hl.config({
    binds = {
...
hl.bind(mainMod .. " + SHIFT + V", hl.dsp.exec_cmd("cliphist list | rofi -dmenu | cliphist decode | wl-copy")) -- clipboard history
hl.bind(mainMod .. " + backslash", hl.dsp.exec_cmd("rofi -show calc -modi calc -no-show-match -no-sort")) -- calculator
hl.bind("CTRL + SHIFT + Escape", hl.dsp.exec_cmd("kitty -e btop")) -- system monitor
...
for key, workspace in pairs(keypadWorkspaces) do
...
hl.bind(mainMod .. " + ALT + mouse:272", hl.dsp.window.resize(), { mouse = true }) -- mainMod + Alt + LMB resizing for laptops
...
-- Auto-import is managed by the hyprland.performanceMode option in your flake
require("imports")
